...
public class FHOTNM2000OTN3Migrator  extends AbstractDBFLoader {
    ...
    List<CMP_CTP> cMpCtpsFromCC = new ArrayList<CMP_CTP>();
...
//	Map<String, CRoute> cc_routeMap = new HashMap<String, CRoute>();
    ...
    public void doExecute() throws Exception {
        ...
        if (migrateLogical) {
            ...
            migrateCTP();
            getLogger().info("烽火SDH，201端口解析：");
            migrate201PtpForFHSDH();
...
//                    }
                    ...
                    //20171122:如果一端是201 FTP下的ctp，另外一端是非201的端口，而非ctp
                    //则该条记录，放入端口绑定表ptn_mp_ctp，mpdn取上述非201端口的dn，ctpdn取201 ftp的dn
                    ...
                    String aend = cc.getaEndNameList();
                    String zend = cc.getzEndNameList();
                    if (aend.contains("port=201@CTP") && !zend.contains("port=201") && !zend.contains("CTP")) {
                    	getLogger().info("aend 201 port:" + orginalDn);
                    	getLogger().info("cccs size=" + cccs.size());
                    	List<CMP_CTP> cMpCtps = addCMpCtpByCC(cccs);
                    	getLogger().info("cMpCtps size=" + cMpCtps.size());
                    	
                    	cMpCtpsFromCC.addAll(cMpCtps);
                    }
                    if (zend.contains("port=201@CTP") && !aend.contains("port=201") && !aend.contains("CTP")) {
                    	getLogger().info("zend 201 port:" + orginalDn);
                    	getLogger().info("cccs size=" + cccs.size());
                    	List<CMP_CTP> cMpCtps = addCMpCtpByCC(cccs);
                    	getLogger().info("cMpCtps size=" + cMpCtps.size());
                    ...
                    //20171122:如果其中一端是端口，而非ctp，则不存到c_crossconnect，丢弃
                    if (!aend.contains("CTP") || !zend.contains("CTP")) {
                    	getLogger().info("drop cc:" + orginalDn);
                    	continue;
                    }
...
//                    System.out.println("ccc = " + ccc);
    ...
    private List<CMP_CTP> addCMpCtpByCC(List<CCrossConnect> cccs) {
    	List<CMP_CTP> cMpCtps = new ArrayList<>();
    	if (Detect.notEmpty(cccs)) {
    		for (CCrossConnect ccc : cccs) {
    			CMP_CTP cMpCtp = new CMP_CTP();
    			if (ccc.getAend().contains("port=201@CTP")) {
    				cMpCtp.setCtpDn(ccc.getAend());
        	    	cMpCtp.setPtpDn(ccc.getZend());
        	    	cMpCtp.setDn(ccc.getZend() + "<>" + ccc.getAend());
    			} else {
    				cMpCtp.setCtpDn(ccc.getZend());
        	    	cMpCtp.setPtpDn(ccc.getAend());
        	    	cMpCtp.setDn(ccc.getAend() + "<>" + ccc.getZend());
    			}

    	    	cMpCtp.setIsUsed(1);
    	    	cMpCtp.setEmsName(emsdn);
    	    	
    	    	cMpCtps.add(cMpCtp);
    	    	
    	    	getLogger().info("add cMpCtp:" + cMpCtp.getDn());
    		}
    	}
...
//	}
    ...
    private List<CStaticRoute> addStaticRouteInETD1(){
    	getLogger().info("add StaticRoute for Etd1, start...");
    	
    	List<Equipment> allEquipments = sd.queryAll(Equipment.class);
    	if (Detect.notEmpty(allEquipments)) {
    		getLogger().info("All equipments size:" + allEquipments.size());
    	} else {
    		getLogger().info("All equipments size: 0");
    	}
    	
    	List<CEquipment> etd1Equipments = this.findObjects(Equipment.class, 
    			"select c from CEquipment c where c.nativeEMSName = 'ETD1' and c.emsName = '" + emsdn + "'");
    	List<CStaticRoute> allStaticRoutes = new ArrayList<>();
    	if (Detect.notEmpty(etd1Equipments)) {
    		getLogger().info("ETD1 equipments size:" + etd1Equipments.size());
    		for (CEquipment equipment : etd1Equipments) {
    			List<CPTP> ptps = this.findObjects(PTP.class, "select c from CPTP c where c.parentDn = '" + equipment.getDn() + "'");
    			List<CStaticRoute> staticRoutes = new ArrayList<>();
    			if (Detect.notEmpty(ptps)) {
    				getLogger().info("ETD1 ptp in [" + equipment.getDn() + "] size:" + etd1Equipments.size());
    				if (etd1Equipments.size() == 1) {
    					continue;
    				} else {
    					int num = (int) Math.floor(etd1Equipments.size()/2);
        				for (int i=1;i<=num;i++) {
        					CPTP aptp = new CPTP();
        					CPTP zftp = new CPTP();
        					for (CPTP ptp : ptps) {
        						if (ptp.getDn().contains("PTP") && ptp.getDn().endsWith("port="+i)
        								&& !ptp.getDn().contains("port=201")) {
        							aptp = ptp;
        						}
        						if (ptp.getDn().contains("FTP") && ptp.getDn().endsWith("port="+i)
        								&& !ptp.getDn().contains("port=201")) {
        							zftp = ptp;
        						}
        					}
        					if (aptp != null && Detect.notEmpty(aptp.getDn()) && zftp != null && Detect.notEmpty(zftp.getDn())) {
        						CStaticRoute cStaticRoute = new CStaticRoute();
        	                    cStaticRoute.setAptp(aptp.getDn());
        	                    cStaticRoute.setZptp(zftp.getDn());
        	                    cStaticRoute.setDn(aptp.getDn()+"<>"+zftp.getDn());
        	                    cStaticRoute.setEmsName(emsdn);
        	                    staticRoutes.add(cStaticRoute);
        					}
        				}
        				
        				getLogger().info("ETD1 staticRoutes in [" + equipment.getDn() + "] size:" + staticRoutes.size());
        				allStaticRoutes.addAll(staticRoutes);
    				}
    				
    			} else {
    				getLogger().info("ETD1 ptp in [" + equipment.getDn() + "] size: 0");
    			}
    			
    		}
    		
    	} else {
    		getLogger().info("ETD1 equipments size: 0");
    	}
    	
    	getLogger().info("all StaticRoutes size:" + allStaticRoutes.size());
    	getLogger().info("add StaticRoute for Etd1, end...");
...
//        HashMap<String,String> lp_mp_map = new HashMap<String, String>();
        ...
        try {
            ...
            //20171124:烽火SDH的ETD1板卡下，把fe口和wan口根据序号成对生成CStaticRoute
            List<CStaticRoute> allStaticRoutes = addStaticRouteInETD1();
            if (Detect.notEmpty(allStaticRoutes)) {
            	di.insert(allStaticRoutes);
            }
...
//            }
            ...
            //20171122:新增CMP_CTP时，把解析crossconnect的特定数据也一并保存
            getLogger().info("fhSDH mp_ctp from cc, size=" + (Detect.notEmpty(cMpCtpsFromCC)?cMpCtpsFromCC.size():0));
            cmp_ctps.addAll(cMpCtpsFromCC);
