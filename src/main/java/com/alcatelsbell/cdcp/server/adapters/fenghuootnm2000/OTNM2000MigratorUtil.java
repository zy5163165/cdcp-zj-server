 ...
 */
public class OTNM2000MigratorUtil {
    ...
    public static CCrossConnect createCrossConnect (String emsDn, R_TrafficTrunk_CC_Section route){
        CCrossConnect des = new CCrossConnect();
        des.setAptp(DNUtil.extractPortDn(route.getaEnd()));
        des.setZptp(DNUtil.extractPortDn(route.getzEnd()));
        des.setAend(route.getaEnd());
        des.setZend(route.getzEnd());

        des.setDn(route.getCcOrSectionDn());

        des.setCollectTimepoint(route.getCreateDate());

   //     des.setDirection(route.get);
        //TODO
...
//    }
    ...
    public static CSection transSection(Section section) {
        ...
        if (rate != null) {
            ...
            try {
                ...
                LogUtil.error(OTNM2000MigratorUtil.class, "Unknown rate :" + rate);
            }
    ...
    public static List<CCrossConnect> transCCS( CrossConnect  cc,String emsdn) {
        ...
        if (actps.length == zctps.length  && actps.length == 1) {
            ...
            if (!ccc.getAend().equals(ccc.getZend())){
            	newCCs.add(ccc);
            }
        }
